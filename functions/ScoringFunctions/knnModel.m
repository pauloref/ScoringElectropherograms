function knnModel(obj,Distances,Threshold,CutOff,wavelet,WaveletThreshold)
...
model = load('knn_w_20.mat'); % load model
...
for k=list'
    ...
    for j=1:length(LOCS)
        ...
        try
        peak_data(j,:) = [time_val,filtered_standard(LOC-window_size:LOC+window_size),Ww,Pp];
        catch
           continue; 
        end
    
    end
    labels = model.knnModel.predictFcn(peak_data); %predict output from model
