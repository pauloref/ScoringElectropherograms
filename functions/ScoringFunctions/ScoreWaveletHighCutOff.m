function ScoreWaveletHighCutOff(obj,Distances,Signal,CutStandard,Standard,CutOff,wavelet,WaveletThreshold)
...
for k=[obj.WellList.WellList{:}]
    ...
    for j=1:4
       ...
       %Standard(i,(pk-window):(pk+window))
        %We now use the dedicated function to find the peak in the window
        %[PeakPos(j,1), PeakPos(j,2)]=max(Standard(i,(pk-window):(pk+window)) );
        ...
        %Find the signal maximum around the standard peak (pk)
        %[SigPeak(j,1), SigPeak(j,2)]=max(Signal(i,(pk-window):(pk+window)) );
